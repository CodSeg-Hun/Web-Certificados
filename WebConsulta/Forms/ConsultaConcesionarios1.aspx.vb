...
Public Class ConsultaConcesionarios1
...
#Region "Procesos"
    ...
    Protected Sub BtnBusquedaAvanzada_Click(sender As Object, e As EventArgs) Handles BtnBusquedaAvanzada.Click
        Try
            ...
            'Me.RadFilter1.RootGroup.AddExpression(New RadFilterContainsFilterExpression("CHASIS") With { .Value = Me.txtbusqueda.Text.Trim() })
            'Dim group As New RadFilterGroupExpression()
            'group.GroupOperation = RadFilterGroupOperation.[Or]
            'RadFilter1.RootGroup.AddExpression(group)
            'Me.RadFilter1.RootGroup.AddExpression(New RadFilterContainsFilterExpression("VEHICULO") With {.Value = Me.txtbusqueda.Text.Trim()})
            'RadFilter1.RootGroup.AddExpression(group)
            'Me.RadFilter1.RootGroup.AddExpression(New RadFilterContainsFilterExpression("PLACA") With {.Value = Me.txtbusqueda.Text.Trim()})
            'RadFilter1.RootGroup.AddExpression(group)
            'Me.RadFilter1.RootGroup.AddExpression(New RadFilterContainsFilterExpression("CLIENTE") With {.Value = Me.txtbusqueda.Text.Trim()})
            'RadFilter1.RootGroup.AddExpression(group)
            'Me.RadFilter1.RootGroup.AddExpression(New RadFilterContainsFilterExpression("PRODUCTO") With {.Value = Me.txtbusqueda.Text.Trim()})
            'RadFilter1.RootGroup.AddExpression(group)
            'Me.RadFilter1.RootGroup.AddExpression(New RadFilterContainsFilterExpression("ORDEN_SERVICIO") With {.Value = Me.txtbusqueda.Text.Trim()})
            'RadFilter1.RootGroup.AddExpression(group)
            'RadFilter.RemoveGroupCommandName.[Or]()
            'RadListView1.DataBind()
